...
async function ed25519KeyPairFromSeed(seed) {
  ...
  pack(pk, p);
  const sk = new Uint8Array(64);
  sk.set(seed.subarray(0, 32), 0);
  sk.set(pk, 32);
  return { publicKey: pk, secretKey: sk };
}

async function ed25519Sign(msg, sk) {
  const seed = sk.subarray(0, 32);
  const pkcs8 = new Uint8Array(48);
  pkcs8.set(ED25519_PKCS8_HEADER, 0);
  pkcs8.set(seed, 16);
  const key = await crypto.subtle.importKey("pkcs8", pkcs8, "Ed25519", false, [
    "sign",
  ]);
...
async function ed25519Open(sm, pk) {
  if (sm.length < 64) return null;
  const sig = sm.subarray(0, 64);
  const msg = sm.subarray(64);
  const key = await crypto.subtle.importKey("raw", pk, "Ed25519", false, [
    "verify",
  ]);
  const valid = await crypto.subtle.verify("Ed25519", key, sig, msg);
  return valid ? new Uint8Array(msg) : null;
}

async function ed25519DetachedVerify(msg, sig, pk) {
  const key = await crypto.subtle.importKey("raw", pk, "Ed25519", false, [
    "verify",
  ]);
...
nacl.sign = async function (msg, secretKey) {
  checkArrayTypes(msg, secretKey);
  if (secretKey.length !== crypto_sign_SECRETKEYBYTES)
    throw new Error("bad secret key size");
  return ed25519Sign(msg, secretKey);
};

nacl.sign.open = async function (signedMsg, publicKey) {
  checkArrayTypes(signedMsg, publicKey);
  if (publicKey.length !== crypto_sign_PUBLICKEYBYTES)
    throw new Error("bad public key size");
  return ed25519Open(signedMsg, publicKey);
};

nacl.sign.detached = async function (msg, secretKey) {
  const sm = await nacl.sign(msg, secretKey);
  return sm.subarray(0, 64);
};

nacl.sign.detached.verify = async function (msg, sig, publicKey) {
  checkArrayTypes(msg, sig, publicKey);
  if (sig.length !== crypto_sign_BYTES) throw new Error("bad signature size");
  if (publicKey.length !== crypto_sign_PUBLICKEYBYTES)
    throw new Error("bad public key size");
  return ed25519DetachedVerify(msg, sig, publicKey);
};
