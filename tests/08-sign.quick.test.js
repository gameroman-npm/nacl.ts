...
describe("nacl.sign.keyPair", function () {
  it("should generate valid key pair", async function () {
    ...
    assert.ok(
      keys.secretKey.length === nacl.sign.secretKeyLength,
      "has secret key",
    );
    assert.ok(
      keys.publicKey.length === nacl.sign.publicKeyLength,
      "has public key",
    );
    assert.notEqual(keys.secretKey.toBase64(), keys.publicKey.toBase64());
    const newKeys = await nacl.sign.keyPair();
    assert.notEqual(
...
describe("nacl.sign.keyPair.fromSecretKey", function () {
  it("should derive same key pair from secret key", async function () {
    const k1 = await nacl.sign.keyPair();
...
describe("nacl.sign.keyPair.fromSeed", function () {
  it("should generate consistent key pairs from seed", async function () {
    const seed = nacl.randomBytes(nacl.sign.seedLength);
    const k1 = await nacl.sign.keyPair.fromSeed(seed);
    const k2 = await nacl.sign.keyPair.fromSeed(seed);
    ...
    const k3 = await nacl.sign.keyPair.fromSeed(seed2);
...
describe("nacl.sign and nacl.sign.open", function () {
  it("should sign and verify messages", async function () {
    const k = await nacl.sign.keyPair();
    const m = new Uint8Array(100);
    let i;
    for (i = 0; i < m.length; i++) m[i] = i & 0xff;
    const sm = await nacl.sign(m, k.secretKey);
    assert.ok(
      sm.length > m.length,
      "signed message length should be greater than message length",
    );
    let om = await nacl.sign.open(sm, k.publicKey);
    assert.deepEqual(om, m);
    await assert.rejects(nacl.sign.open(sm, k.publicKey.subarray(1)), Error);
    const badPublicKey = new Uint8Array(k.publicKey.length);
    om = await nacl.sign.open(sm, badPublicKey);
    assert.equal(
      om,
      null,
    ...
    om = await nacl.sign.open(sm, k.publicKey);
    assert.equal(
      om,
      null,
...
describe("nacl.sign.detached and nacl.sign.detached.verify", function () {
  it("should create and verify detached signatures", async function () {
    const k = await nacl.sign.keyPair();
    const m = new Uint8Array(100);
    let i;
    for (i = 0; i < m.length; i++) m[i] = i & 0xff;
    const sig = await nacl.sign.detached(m, k.secretKey);
    assert.ok(
      sig.length === nacl.sign.signatureLength,
      "signature must have correct length",
    );
    let result = await nacl.sign.detached.verify(m, sig, k.publicKey);
    ...
    const badPublicKey = new Uint8Array(k.publicKey.length);
    result = await nacl.sign.detached.verify(m, sig, badPublicKey);
    assert.equal(
      result,
      false,
    ...
    result = await nacl.sign.detached.verify(m, sig, k.publicKey);
