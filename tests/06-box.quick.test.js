...
describe('nacl.box.keyPair', function() {
  it('should generate valid key pair', function() {
    ...
    assert.ok(keys.secretKey && keys.secretKey.length === nacl.box.secretKeyLength, 'has secret key');
    assert.ok(keys.publicKey && keys.publicKey.length === nacl.box.publicKeyLength, 'has public key');
...
describe('nacl.box and nacl.box.open', function() {
  it('should encrypt and decrypt message', function() {
    ...
    assert.equal(new TextDecoder().decode(clientMsg), new TextDecoder().decode(msg));
    ...
    assert.equal(new TextDecoder().decode(serverMsg), new TextDecoder().decode(msg));
  });
});
...
describe('nacl.box.open with invalid box', function() {
  it('should return null', function() {
    ...
    var serverKeys = nacl.box.keyPair();
    var nonce = new Uint8Array(nacl.box.nonceLength);
    assert.equal(nacl.box.open(new Uint8Array(0), nonce, serverKeys.publicKey, clientKeys.secretKey), null);
    assert.equal(nacl.box.open(new Uint8Array(10), nonce, serverKeys.publicKey, clientKeys.secretKey), null);
    assert.equal(nacl.box.open(new Uint8Array(100), nonce, serverKeys.publicKey, clientKeys.secretKey), null);
  });
});
...
describe('nacl.box.open with invalid nonce', function() {
  it('should return null when nonce is wrong', function() {
    ...
    assert.equal(new TextDecoder().decode(nacl.box.open(box, nonce, serverKeys.publicKey, clientKeys.secretKey)),
            new TextDecoder().decode(msg));
    nonce[0] = 255;
    assert.equal(nacl.box.open(box, nonce, serverKeys.publicKey, clientKeys.secretKey), null);
  });
});
...
describe('nacl.box.open with invalid keys', function() {
  it('should return null when keys are wrong', function() {
    ...
    assert.equal(new TextDecoder().decode(nacl.box.open(box, nonce, serverKeys.publicKey, clientKeys.secretKey)),
            new TextDecoder().decode(msg));
    assert.equal(new TextDecoder().decode(nacl.box.open(box, nonce, clientKeys.publicKey, serverKeys.secretKey)),
            new TextDecoder().decode(msg));
    var badPublicKey = new Uint8Array(nacl.box.publicKeyLength);
    assert.equal(nacl.box.open(box, nonce, badPublicKey, clientKeys.secretKey), null);
    var badSecretKey = new Uint8Array(nacl.box.secretKeyLength);
    assert.equal(nacl.box.open(box, nonce, serverKeys.publicKey, badSecretKey), null);
  });
});
